<?php
    ...
    if($btnDispatchClicked == true) {
		...
		$sql = "INSERT INTO `incident`( `caller_name`, `phone_number`, `incident_type_id`, `incident_locations`, `incident_desc`, `incident_status_id`, `time_called`) VALUES ('" . $callerName . "','" . $contactNo . "','" . $typeOfIncident . "','" . $locationOfIncident . "','" . $descriptionOfIncident . "','" . $incidentStatus . "',now());";
		//echo $sql;
		$conn = new mysqli(DB_SERVER,DB_USER,DB_PASSWORD,DB_DATABASE);
		$innsertIncidentSuccess = $conn->query($sql);
		if($innsertIncidentSuccess == false) {
...
<div class="container" style="width: 900px">
  ...
  <section class="mt-3">
    <form action="<?php echo htmlentities($_SERVER["PHP_SELF"])?>" method="post">
	  ...
	  <div class="form-group row">
        <label for="patrolCar" class="col-sm-4 col-form-label">Patrol Car(s)</label>
        <div class="col-sm-8">
      ...
      <div class="form-group row">
        <div class="offset-sm-4 col-sm-8">
          <button type="submit" class="btn btn-primary" name="btnProcessCall" id="submit">Dispatch</button>
        </div>
		</div>
